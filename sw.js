...
const LLM_CACHE = 'eiti-llm-v1'; // WebLLM (MLC) кэш — защищён от удаления при обновлении SW

// v10.4 FIX: require-corp нужно для SharedArrayBuffer (Transformers.js WASM multi-thread)
// v12.6 FIX: не добавляем COEP к HTML-странице — блокирует динамический import() с CDN
function addCoopHeaders(response, request) {
    if (!response || response.type === 'opaque') return response;
    var url = request ? request.url : '';
    var isHtml = url.endsWith('.html') || url.endsWith('/') || url === self.registration.scope;
    // Для HTML-страницы — только COOP, без COEP (чтобы CDN import() работал)
    if (isHtml) {
    ...
    e.waitUntil(
        caches.open(CACHE_NAME).then(function(cache) {
            return cache.addAll(ASSETS).catch(function(err) {
                console.warn('[SW v12.4] Cache addAll partial fail:', err);
            });
        })
    );
});
        ...
        caches.keys().then(function(keys) {
            return Promise.all(
                keys.filter(function(k) {
                    return k !== CACHE_NAME && k !== TRANSFORMERS_CACHE && k !== LLM_CACHE;
                }).map(function(k) {
